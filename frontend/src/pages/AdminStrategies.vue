...
const breadcrumbItems = computed(() => {
  const items = [{ label: 'Home', to: '/' }, { label: 'Admin', to: '/admin' }]
  if (mode.value === 'list') {
    items.push({ label: 'Strategies' })
  } else {
    items.push({ label: 'Strategies', onClick: () => { mode.value = 'list' } })
    items.push({ label: editingId.value ? 'Edit' : 'Create' })
  }
  return items
})
